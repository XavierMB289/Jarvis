...
class AIConnect:
	...
	def call(self, user_input: str) -> str:
		...
		"""
		self.inputs.append({"role": "user", "content": user_input})
			...
			if item.type == "function_call":
				...
				match item.name:
					case "create_file":
						...
						self.call_handler.create_file(file_path, file_data)
					case "read_file":
						file_path = loaded_json["file_path"]
						self.call_handler.read_file(file_path)
					case "delete_file":
						file_path = loaded_json["file_path"]
						self.call_handler.delete_file(file_path)
					case "create_connections":
						message = loaded_json["message"]
						self.call_handler.create_connections(message)
					case "send_file":
						file_path = loaded_json["file_path"]
						email = loaded_json["email"]
						self.call_handler.send_file(file_path, email)
					case "send_email":
						subject = loaded_json["subject"]
						message = loaded_json["message"]
						email = loaded_json["email"]
						self.call_handler.send_email(subject, message, email)
					case "text_user":
						message = loaded_json["message"]
